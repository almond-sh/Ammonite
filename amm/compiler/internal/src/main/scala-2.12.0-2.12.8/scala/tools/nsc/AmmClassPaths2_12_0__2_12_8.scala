 ...
 */
final class UrlZipArchiveClassPath2_12_0__2_12_8(val zipUrl: URL)
    extends ClassPath
    with NoSourcePaths {
  ...
  def packages(inPackage: String): Seq[PackageEntry] = packagesIn(inPackage)
  def list(inPackage: String): ClassPathEntries = listIn(inPackage)
  def classes(inPackage: String): Seq[ClassFileEntry] = filesIn(inPackage)
  def hasPackage(pkg: String): Boolean = findDirEntry(pkg).isDefined
}

/** An aggregate class path cut down to the classes a white list lets through. */
final class WhiteListClassPath2_12_0__2_12_8(
