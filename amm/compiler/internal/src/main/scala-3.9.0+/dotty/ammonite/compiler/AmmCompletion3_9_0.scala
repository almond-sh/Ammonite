...
/** Ammonite's completions, for the compilers of 3.9.0 and later. */
object AmmCompletion3_9_0 {
  ...
  def computeCompletions(
    ...
    val completions = path match {
      ...
      case (_: untpd.ImportSelector) :: Import(expr, _) :: _ =>
        completer.directMemberCompletions(expr)
      case _                                                 =>
        // Completion.CompletionResult.names carries the site each denotation was found at
        // since 3.9.0 - we only care about the denotations themselves
        completer.scopeCompletions.names.map((name, denot) => name -> denot.denots) ++ {
